#!/usr/bin/python3
################################################ x_err, y_err, no angular velocity ################################################
import rospy
...
class KalmanMultiTracker:    
    ...
    def publish_tracked_people(self, now):
        ...
        if not transform_available:
            ...
            for person in self.objects_tracked:
                if person.is_person == True:
                    ...
                    # Different in the 08 March code
        ...
        # Call move function only if a person is detected 
    ...
    def move(self,x, y, yaw, yaw2):
        ...
        print('Person position: ',x, y, np.rad2deg(yaw))
        ...
        print('Robot position: ',self.x_robot, self.y_robot, np.rad2deg(self.yaw_robot))
        ...
        if abs(round(x,2) - round(self.x_robot,2)) <= 0.1:
            x_err = 0
            # print('x_err reset /////////////////----------------')
        else:
            x_err =  0.5

        if abs(round(y,2) - round(self.y_robot,2)) <= 0.1:
            y_err = 0
            # print('y_err reset /////////////////----------------')
        else:
            y_err = 0.5
        ...
        # yaw_to_rotate = math.atan2((y-self.y_robot),(x-self.x_robot)) - self.yaw_robot

        
        # if abs(yaw_to_rotate < np.deg2rad(2.0)):
        #     yaw_to_rotate = 0
        #     print('Yaw < 2.0 deg')

        print('yaw to rotate : ',np.rad2deg(yaw_to_rotate))
        print('\n')
        #Scaling the command_velocity
        if round(x,2)!=0 or round(y,2)!=0:
            tw.linear.x = 1.0#(1/np.deg2rad(120)) * v * (np.deg2rad(120) - abs(yaw_to_rotate))
            tw.linear.y = -0.5
    ...
    def robot_pose_callback(self,msg):
        ...
        print('Yaw in callback : ', np.rad2deg(self.yaw_robot))
