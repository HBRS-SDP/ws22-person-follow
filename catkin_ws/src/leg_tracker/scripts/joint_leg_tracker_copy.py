...
class KalmanMultiTracker:    
    ...
    def __init__(self):      
        ...
        self.max_leg_pairing_dist = rospy.get_param("max_leg_pairing_dist", 0.8) # 0.5
        ...
        self.dist_travelled_together_to_initiate_leg_pair = rospy.get_param("dist_travelled_together_to_initiate_leg_pair", 0.5) # 0.3
        ...
        self.robot_pose_sub = rospy.Subscriber('/hsrb/odom', Odometry, self.robot_pose_callback)
    ...
    def publish_tracked_objects(self, now):
        ...
        if self.use_scan_header_stamp_for_tfs:
            ...
            try:
                ...
                print('Exception at line 523 : ',str(e))
    ...
    def rotate_robot(self, yaw, yaw2, y, x):
        ...
        # print('person yaw2 : ',yaw2)
        print('Robot position: ',self.x_robot, self.y_robot, self.yaw_robot, np.rad2deg(self.yaw_robot))
        output_pose = self.transform_pose(KalmanMultiTracker.pose_var, 'base_link', 'odom')
        print('Transformed pose : \n',output_pose)


        if y < 0:
            if x != 0 :
                yaw_person = min(yaw, yaw2)
                print('y<0 Min of Yaw taken here : ', np.rad2deg(yaw_person))
            else:
                yaw_person = 0
                print(' y<0 but x==0 : ',yaw_person)
        elif y > 0:
            if x != 0 :
                yaw_person = max(yaw, yaw2)
                print('y>0 Max of Yaw taken here : ', np.rad2deg(yaw_person))
            else:
                yaw_person = 0 
                print(' y>0 but x==0 : ',yaw_person)

        # Determine direction and to rotate or not based on the distance from robot(wrt Odom) to person(wrt base_link)...
        if round(y,1) != 0.0:
            if abs(round(y,1)) <= 0.5:
                if round(yaw_person,0) < round(self.yaw_robot,0):
                    ang_vel = -0.5
                elif round(yaw_person,0) > round(self.yaw_robot,0):
                    ang_vel = 0.5
                elif round(np.yaw_person,0) == round(self.yaw_robot,0):
                    ang_vel = 0
            elif abs(round(y,1)) > 0.5: # x_person > 0.5
                if y < 0:
                    ang_vel = -0.5
                elif y > 0:
                    ang_vel = 0.5
        ...
        # if round(self.yaw_robot,0) == round(yaw_person,0):
        #     ang_vel = 0
    ...
    def move(self,x, y, yaw, yaw2):
        ...
        print('is rotate : ',KalmanMultiTracker.is_rotate)
        print('flag x : ',KalmanMultiTracker.flag_x)
        print('flag y : ',KalmanMultiTracker.flag_y)

        print('Yaw : {} | Yaw2 : {}'.format(np.rad2deg(yaw),np.rad2deg(yaw2)))

        print('Person position - base_link: ',x, y)
        if KalmanMultiTracker.is_rotate == True:
            KalmanMultiTracker.is_rotate = False
            self.rotate_robot(yaw,yaw2,y,x)
            KalmanMultiTracker.acc_sum_x = 0
            KalmanMultiTracker.acc_sum_y = 0
            print('IF')
        else:
            print('ELSE')
            # Convert/Transfom all person wrt Odom from base_link
            x_person = x #+ self.x_robot  # wrt Odom   # x_person = x
            y_person = y #+ self.y_robot  # wrt Odom
            # yaw_person = yaw_person - self.yaw_robot  # wrt. Odom

            print('Person position - odom: ',x_person, y_person)#, yaw_person, np.rad2deg(yaw_person))
            print('Prev Robot position ',self.x_prev_robot, self.y_prev_robot)#, self.yaw_robot, np.rad2deg(self.yaw_robot))
            print('Robot position: ',self.x_robot, self.y_robot, self.yaw_robot, np.rad2deg(self.yaw_robot))

            # if abs(round(x_dist,1)) == 1.0 or abs(round(y_dist == 1.0)):
            # if abs(round(round(x_person - self.x_robot,1),1)) == 1.0:

            KalmanMultiTracker.acc_sum_x += round(self.x_prev_robot-self.x_robot,1)
            KalmanMultiTracker.acc_sum_y += round(self.y_prev_robot-self.y_robot,1)
            # if abs(round(self.x_prev_robot-self.x_robot,1)) == abs(round(x,1)):
            if abs(round(KalmanMultiTracker.acc_sum_x,1)) == abs(round(x,1)):
                KalmanMultiTracker.flag_x = True
                print('1')
                tw.linear.x = 0
            elif abs(round(KalmanMultiTracker.acc_sum_x,1)) != abs(round(x,1)):
                if KalmanMultiTracker.flag_x == False:
                    print('3')
                    x_dist = round(x_person - self.x_robot,1)
                    tw.linear.x = x_dist * k_p


            # if abs(round(round(y_person - self.y_robot,1) == 1.0)):
            # if abs(round(self.y_prev_robot-self.y_robot,1)) == abs(round(y,1)):
            if abs(round(KalmanMultiTracker.acc_sum_y,1)) == abs(round(y,1)):
                KalmanMultiTracker.flag_y = True
                print('2')
                tw.linear.y = 0
            elif abs(round(KalmanMultiTracker.acc_sum_y,1)) != abs(round(y,1)):
                if KalmanMultiTracker.flag_y == False:
                    print('4')
                    y_dist = round(y_person - self.y_robot,1)
                    tw.linear.y = y_dist * k_p


            # if abs(round(round(x_person - self.x_robot,1) ,1)) != 1.0:
            # if abs(round(self.x_prev_robot-self.x_robot,1)) != abs(round(x,1)):
            
            # if abs(round(round(y_person - self.y_robot,1),1)) != 1.0:
            # if abs(round(self.y_prev_robot-self.y_robot,1)) != abs(round(y,1)):
            
            print('accumulated sum x : {} | y : {}'.format(KalmanMultiTracker.acc_sum_x,KalmanMultiTracker.acc_sum_y))
            # print('x-dist : {} | y-dist : {}'.format(round(self.x_prev_robot-self.x_robot,1) ,round(self.y_prev_robot-self.y_robot,1)))

            # if abs(x_person - self.x_robot) <= 0.1:
            #     x_dist = 0
            #     # print('x_err reset /////////////////----------------')
            

            # if abs(y_person - self.y_robot) <= 0.1:
            #     y_dist = 0
            #     # print('y_err reset /////////////////----------------')
            
            

            # print('Person to robot : ',np.rad2deg(math.atan2((y-self.y_robot),(x-self.x_robot))))

            # # yaw_to_rotate = math.atan2((-y_person+self.y_robot),(-x_person+self.x_robot)) 
            # # print('Atan2 : ', yaw_to_rotate, np.rad2deg(yaw_to_rotate))

            
            
            # # tw.angular.z = ang_vel
            
            # # Stopping criteria for rotation
            # # if abs(round(yaw_to_rotate,1) - round(self.yaw_robot,1)) <= np.deg2rad(2.0):
            # #     ang_vel = 0

            # # if abs(round(self.yaw_robot,1) - round(yaw_person,1)) <= np.deg2rad(2.0):
            # #     ang_vel = 0 
                
            #Scaling the command_velocity
            # if round(x_person,1)!=0 or round(y_person,1)!=0:
            #     tw.linear.x = x_dist*k_p #(1/np.deg2rad(120)) * v * (np.deg2rad(120) - abs(yaw_to_rotate))
            #     tw.linear.y = y_dist*k_p
            #     # tw.angular.z = 0
            # else:
            #     tw.linear.x = 0 #(1/np.deg2rad(120)) * v * (np.deg2rad(120) - abs(yaw_to_rotate))
            #     tw.linear.y = 0
                # tw.angular.z = 0
            
            # print('Angular Velocity : {} | linear.x : {} | linear.y : {}'.format(ang_vel, x_dist*k_p,y_dist*k_p))
            #     # tw.angular.z = round(yaw_to_rotate,1)*k_p
            # # else:
            # #     tw.angular.z = 0
            # # else:
            #     # tw.linear.x = 0
            ...
            self.x_prev_robot = self.x_robot
            self.y_prev_robot = self.y_robot  
    ...
    def transform_pose(self,input_pose, from_frame, to_frame):
        ...
        #     try:
        #         trans = tfBuffer.lookup_transform(from_frame, to_frame, rospy.Time())
        #     except (tf2_ros.LookupException, tf2_ros.ConnectivityException, tf2_ros.ExtrapolationException):
        #         rate.sleep()
        #         continue
        #     print('Trans : \n',trans)
