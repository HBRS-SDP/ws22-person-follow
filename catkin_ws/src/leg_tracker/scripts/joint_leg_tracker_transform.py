...
class KalmanMultiTracker:    
    ...
    def rotate_robot(self, yaw, yaw2, y, x):
        ...
        if round(y,1) != 0.0:
            if abs(round(y,1)) <= 0.5:
                if round(yaw_person,0) < round(self.yaw_robot,0):
                    KalmanMultiTracker.ang_vel = -0.5
                elif round(yaw_person,0) > round(self.yaw_robot,0):
                    KalmanMultiTracker.ang_vel = 0.5
                elif round(yaw_person,0) == round(self.yaw_robot,0):
                    KalmanMultiTracker.ang_vel = 0
        ...
        tw.angular.z = KalmanMultiTracker.ang_vel

        print('Ang vel : ',KalmanMultiTracker.ang_vel)
        # print('Robot position: ',self.x_robot, self.y_robot, self.yaw_robot, np.rad2deg(self.yaw_robot))
        print('\n\n')
    ...
    def move(self):
        ...
        # orientation_list = [person_transformed_pose.orientation.x,person_transformed_pose.orientation.y,person_transformed_pose.orientation.z,person_transformed_pose.orientation.w]
        # _, _, person_transformed_yaw = tf.transformations.euler_from_quaternion(orientation_list)

        # self.rotate_robot(yaw, yaw2, y, x)
        # person_transformed_yaw = math.atan2(y,x)
        person_transformed_yaw = 0
        y_vel, x_vel, ang_vel = 0, 0, 0
        # if y < 0:
        #     if x != 0 :
        #         person_transformed_yaw = min(yaw, yaw2)
        #         print('y<0 Min of Yaw taken here : ', np.rad2deg(person_transformed_yaw))
        #     else:
        #         person_transformed_yaw = 0
        #         print(' y<0 but x==0 : ',person_transformed_yaw)
        # elif y > 0:
        #     if x != 0 :
        #         person_transformed_yaw = max(yaw, yaw2)
        #         print('y>0 Max of Yaw taken here : ', np.rad2deg(person_transformed_yaw))
        #     else:
        #         yaw_pperson_transformed_yawerson = 0 
        #         print(' y>0 but x==0 : ',person_transformed_yaw)

        print('Person trans yaw : ', person_transformed_yaw, np.rad2deg(person_transformed_yaw))

        # if round(person_transformed_yaw - self.yaw_robot,0) >= np.deg2rad(-2.0) and round(person_transformed_yaw - self.yaw_robot,0) <= np.deg2rad(2.0):
        #     tw.angular.z = 0
        #     print('11')
        # else:
        #     if round(person_transformed_pose.position.y,1) != 0.0:
        #         if abs(round(person_transformed_pose.position.y,1)) <= 0.5:
        #             if round(np.rad2deg(person_transformed_yaw),0) < round(np.rad2deg(self.yaw_robot),0):
        #                 ang_vel = -0.1
        #                 print('12')
        #             elif round(np.rad2deg(person_transformed_yaw),0) > round(np.rad2deg(self.yaw_robot),0):
        #                 ang_vel = 0.1
        #                 print('13')
        ...
        #                 ang_vel = -0.1
        #                 print('15')
        #             elif person_transformed_pose.position.y > 0:
        #                 ang_vel = 0.1
        #                 print('16')
        #     else: # y_person = 0
        #         ang_vel = 0
        ...
        # else:
        #     tw.angular.z = ang_vel
        # print('angular vel : ', tw.angular.z)
    ...
    def transform_pose(self,input_pose, from_frame, to_frame):
        ...
        # listener.waitForTransform(from_frame, to_frame, tf_time, rospy.Duration(1.0))
...
#     # def move(self,x, y, yaw, yaw2):
    def move1(self):
        ...
        # self.rotate_robot(yaw, yaw2, y, x)
        person_transformed_yaw = math.atan2(person_transformed_pose.position.y,person_transformed_pose.position.x)
        print('Person trans yaw : ', person_transformed_yaw, np.rad2deg(person_transformed_yaw))
        ...
        if round(person_transformed_yaw - self.yaw_robot,0) >= np.deg2rad(-2.0) and round(person_transformed_yaw - self.yaw_robot,0) <= np.deg2rad(2.0):
            ...
            if round(person_transformed_pose.position.y,1) != 0.0:
                if abs(round(person_transformed_pose.position.y,1)) <= 0.5:
                    if round(person_transformed_yaw,0) < round(self.yaw_robot,0):
                        KalmanMultiTracker.ang_vel = -0.5
                        print('12')
                    elif round(person_transformed_yaw,0) > round(self.yaw_robot,0):
                        KalmanMultiTracker.ang_vel = 0.5
                        print('13')
                    elif round(person_transformed_yaw,0) == round(self.yaw_robot,0):
                        print(person_transformed_yaw, self.yaw_robot)
                        KalmanMultiTracker.ang_vel = 0
