...
class KalmanMultiTracker:    
    ...
    def __init__(self):      
        ...
        print('Fixed Frame : ',self.fixed_frame)
        self.publish_people_frame = rospy.get_param("publish_people_frame", self.fixed_frame)
        print('Publish_people_frame : ',self.publish_people_frame)
        ...
        self.dist_travelled_together_to_initiate_leg_pair = rospy.get_param("dist_travelled_together_to_initiate_leg_pair", 0.25)
        ...
        # self.go_relative_pos(np.deg2rad(45))   
        ...
        self.pub0 = rospy.Publisher('/hsrb/command_velocity', geometry_msgs.msg.Twist, queue_size=10)
    ...
    def detected_clusters_callback(self, detected_clusters_msg):    
        ...
        #print('Sleep (5)')
        # rospy.sleep(5)
        #print('at line 501 - publish tracked objects')
        #self.publish_tracked_objects(now)
        #print('Sleep for (5)')
        # rospy.sleep(5)
        #print('at line 505 - publish tracked people')
        self.publish_tracked_people(now)
        #print('publish_tracked_people')
    ...
    def publish_tracked_objects(self, now):
        ...
        #print('inside Publish_tracked_objects')
        #print('use_scan_header_stamp_for_tfs : ',self.use_scan_header_stamp_for_tfs)
        if self.use_scan_header_stamp_for_tfs:
            ...
            #print('transform_available : ',transform_available)

        marker_id = 0
        if not transform_available:
            #print('If not transform_available condition')
            rospy.loginfo("Person tracker: tf not avaiable. Not publishing people")
        else:
            #print('In else cond for not transform_available')
            for track in self.objects_tracked:
                if track.is_person:
                    print('if track.is_person then continue')
                    continue
                    
                if track.seen_in_current_scan: #self.publish_occluded or track.seen_in_current_scan: # Only publish people who have been seen in current scan, unless we want to publish occluded people
                    ...
                    marker.id = marker_id
                    print('Object Marker.id : ',marker.id)
                    marker_id += 1
                    ...
                    arker.scale.y = 2*gate_dist_euclid
                    marker.scale.z = 0.01   
                    marker.color.r = 1.0
                    marker.color.g = 1.0
                    marker.color.b = 1.0                
    ...
    def publish_tracked_people(self, now):
        ...
        if not transform_available:
            ...
            for person in self.objects_tracked:
                #print('for person in objects_tracked')
                if person.is_person == True:
                    #print('Person.is_person == True')
                    if self.publish_occluded or person.seen_in_current_scan: # Only publish people who have been seen in current scan, unless we want to publish occluded people
                        ...
                        ps = PointStamped()
                        ps.header.frame_id = self.fixed_frame
                        #print('fixed_frame : ',self.fixed_frame)
                        ps.header.stamp = tf_time
                        ps.point.x = person.pos_x
                        ps.point.y = person.pos_y
                        try:
                            #print('Ps : ',ps)
                            #print('Publish people : ',self.publish_people_frame)
                            #rospy.sleep(5)
                        ...
                        #print('\nQuaternion for +0.4515 yaw is: {}'.format(tf.transformations.quaternion_from_euler(0, 0, 0.4515)))
                        #print('Quaternion for -0.4515 yaw is: {}\n'.format(tf.transformations.quaternion_from_euler(0, 0, -0.4515)))
                        # if yaw <= 0.45 and yaw >=-0.45:
                        #     quaternion = tf.transformations.quaternion_from_euler(0, 0, yaw)
                        #     print('\nYaw (rad) : ',np.rad2deg(yaw))
                        # else:
                        ...
                        #self.move(new_person.pose.position.x,new_person.pose.position.y,yaw,quaternion)
                        #print('---------Robot moved-before adding  markers -------------')
                        ...
                        #     print('for m_id loop in person_tracked')
                        #     marker = Marker()
                        #     marker.header.stamp = now                
                        #     marker.header.frame_id = self.publish_people_frame
                        #     marker.ns = "People_tracked"
                        ...
                        # Yaw given & Position : 0 i.e. only rotates    
                        goal = PoseStamped()
                        goal.header.stamp = rospy.Time.now()
                        goal.header.frame_id = 'base_link'
                        goal.pose.position = Point(0,0,0)
                        # goal.pose.position = Point(temp_x,temp_y,0)

                        quat = tf.transformations.quaternion_from_euler(0,0,temp_yaw)
                        ...
                        print('xxxxxxxxxx-------------ROBOT ROTATED---------------xxxxxxxxxxxx')
                        print('SLEEP START ------------------------')
                        rospy.sleep(3.0)
                        ...
                        # while self.pub0.get_num_connections() == 0:
                        #     rospy.sleep(0.1)
                        ...
                        # self.go_relative_pos(new_person.pose.position.x,new_person.pose.position.y,yaw, quaternion, person.vel_x, marker_id)   
                        # self.move(new_person.pose.position.x,new_person.pose.position.y,yaw2, quaternion, person.vel_x)
                        
                        #self.move()
                    #print('move_x: {}\nmove_y: {}\nmove_yaw: {}\nmove_qaurternion:{}\nRobot Moved....xxxxxxxxxxxxxxxxx.....................'.format(new_person.pose.position.x,new_person.pose.position.y,yaw, quaternion))
                # Clear previously published people markers
                for m_id in range(marker_id):#, self.prev_person_marker_id):
                    print('for m_id loop in person_tracked')
                    marker = Marker()
        ...
        # quat = tf.transformations.quaternion_from_euler(0,0,temp_yaw)
        # self.goal.pose.orientation = Quaternion(*quat)
        # # print('Iteration ')
        # if temp_yaw != 0:
        ...
        # self.move(new_person.pose.position.x,new_person.pose.position.y,yaw, quaternion, person.vel_x)
        #print('self.people_tracked_pub.publish(people_tracked_msg) : ',self.people_tracked_pub.publish(people_tracked_msg))
        #self.move(new_person.pose.position.x,new_person.pose.position.y,yaw, quaternion)
        #print('Robot moved')


    # def move(self, x, y, yaw, quaternion, person_vel_x):
    #     x = x
    #     y = y
    #     yaw = yaw
    #     print('ddddddddddddddddddddddddddddddddddddddd\nmove_x : {}\nmove_y : {}\nmove_yaw : {}\nmove_quaternion : {} '.format(x,y,yaw,quaternion))
    ...
    #     pub0 = rospy.Publisher('/hsrb/command_velocity', geometry_msgs.msg.Twist, queue_size=10)

    #     print('ddddddddddddddddddddddddddddddddddddddd\nmove_x : {}\nmove_y : {}\nmove_yaw : {}\nyaw(rad) : {} \nmove_quaternion : {} '.format(x,y,yaw,np.rad2deg(yaw),quaternion))
    #     pub = rospy.Publisher('goal',PoseStamped,queue_size=10)
    #     while pub.get_num_connections()<2:
    #         rospy.sleep(0.1)
         
    #     # wait to establish connection between the controller
    #     while pub0.get_num_connections() == 0:
    #         rospy.sleep(0.1)
    ...
    #     print('move_x: {}\nmove_y: {}\nmove_yaw: {}\nyaw (rad) : {}\nmove_qaurternion:{}\nRobot Moved....xxxxxxxxxxxxxxxxx.....................'.format(x,y,yaw, np.rad2deg(yaw),quaternion))
    ...
    def go_relative_pos(self, x, y, yaw, quaternion, person_vel_x, marker_id):
        x = x
        y = y
        yaw = yaw
        ...
        try:
            ...
            if abs(np.rad2deg(yaw)) >= 20:
                print(np.rad2deg(yaw))
                if np.rad2deg(yaw)>=20:
                    yaw = yaw - np.deg2rad(20)
                else:
                    yaw = -np.deg2rad(20) - yaw
                print('Yaw Modified ................................................................................')
                print(np.rad2deg(yaw))
                print('-----------------')
            #quat = quaternion
            ...
            print(str(e))
            rospy.logerr('Fail relative go')
        else:
            # self.move(x, y, yaw, quaternion, person_vel_x)
            pass
